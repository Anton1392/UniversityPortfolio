...
struct inode 
{
	...
	// Read_write stuff
	int reader_count;
	struct lock reader_count_lock;

	struct lock w_lock; // Protects rw sema
...
inode_open (disk_sector_t sector) 
{
	...
	lock_init(&inode->reader_count_lock);
...
inode_read_at (struct inode *inode, void *buffer_, off_t size, off_t offset) 
{
	...
	lock_acquire(&inode->reader_count_lock);
	if(++inode->reader_count == 1)
	{
		sema_down(&inode->rw_sema);
	}
	lock_release(&inode->w_lock);
	lock_release(&inode->reader_count_lock);
