...
{
  plist_init(&global_process_list);
}
...
process_execute (const char *command_line)
{
  ...
  // Initiate semaphore to zero
  struct semaphore sync_sema;
  sema_init(&sync_sema, 0);

  // Pass semaphore pointer as argument into process
  arguments.sync_sema = &sync_sema;
  arguments.status = -1;
  ...
  int to_wait = thread_create (debug_name, PRI_DEFAULT, (thread_func*)start_process, &arguments);

  // Wait for sema down, point e done when this goes through.
  if(to_wait != -1)
  {
    sema_down(&sync_sema);
  }
  ...
  /* WHICH thread may still be using this right now? */ // None?
...
start_process (struct parameters_to_start_process* parameters)
{
  ...
  if (success)
  {
    ...
    //printf("%ESPS ARGV[0]: %s", (if_.esp)->argv[0]);
...
//    dump_stack ( PHYS_BASE + 15, PHYS_BASE - if_.esp + 16 );
  ...
  */
  ...
  {
    parameters->status = -1;
    sema_up(parameters->sync_sema);
    thread_exit ();
  }
  else
  {
    parameters->status = thread_current()->tid;
    sema_up(parameters->sync_sema);

    // Add process to process list
    plist_add_process(&global_process_list, thread_current()->name, thread_current()->tid, parameters->parent_pid);
  }
...
process_wait (int child_id)
{
  ...
  struct process_info* p = plist_find_process(&global_process_list, child_id);
  if(p != NULL && cur->tid == p->parent_pid && p->parent_alive)
  {
	  ...
	  p->alive = 0;
	  p->parent_alive = 0;
    plist_remove_process(&global_process_list, -1234); // Hack to clean list from processes.
  }
...
process_cleanup (void)
{
  ...
  struct process_info* status_process = plist_find_process(&global_process_list, thread_current()->tid);
  if(status_process != NULL)
  {
    status = status_process->exit_status;
  }
  ...
  printf("%s: exit(%d)\n", thread_name(), status);
	...
	if(status_process != NULL)
	{
		sema_up(&status_process->wait_sema);
		plist_remove_process(&global_process_list, thread_current()->tid);
	}
...
void* setup_main_stack(const char* command_line, void* stack_top)
{
	...
	line_size = strlen(command_line) + 1; // Include trailing \0.
	...
	// Command line content size + pointers to each argument and 00000000 + (return addr + integer + argv);
	total_size = line_size + (argc + 1) * sizeof(char*) + sizeof(struct main_args);
	STACK_DEBUG("# total_size = %d\n", total_size);


	/* calculate where the final stack top will be located */
	// Phys base minus size, downward growth.
	esp = (struct main_args*)(stack_top - total_size);
	STACK_DEBUG("# stack top = %p\n", esp);

	/* setup return address and argument count */
	esp->ret = 0;
	esp->argc = argc;
	STACK_DEBUG("# set ret and argc to %d and %d\n", 0, argc);

	/* calculate where in the memory the argv array starts */
	//esp->argv = (char**)(stack_top - total_size - (argc+1)*sizeof(char*));
	esp->argv = (char**)(&(esp->argv) + 1);
