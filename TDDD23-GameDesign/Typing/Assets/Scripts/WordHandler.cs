...
public class WordHandler : MonoBehaviour
{
    ...
    public int powerupChance; // 4 percent.
    ...
    void Update()
    {
        // Letters of last frame
        Regex rgx = new Regex("[^a-zA-Z]");
        string input = rgx.Replace(Input.inputString, "");
    ...
    void GetNewWord()
    {
        ...
        // Powerups
        isPowerup = (powerupChance >= Random.Range(1, 100));
    }
    void TypeLetter(char c)
    {
        if (currentWord[chrIndex] == c)
        {
            fm.GoodClick();
            chrIndex++;
            // Word is done
            if (chrIndex >= currentWord.Length)
            {
                if (isPowerup) { ph.GrantPowerup(); }
                GiveAmmo();
                GetNewWord();
            }
        }
        // Punish once per character max.
        else if(chrIndex != lastMistakeIndex)
        {
            fm.BadClick();
            mistakes++;
            lastMistakeIndex = chrIndex;
        }
    }

    void GiveAmmo()
    {
        cannon.Load(currentWord.Length - mistakes);
    }
