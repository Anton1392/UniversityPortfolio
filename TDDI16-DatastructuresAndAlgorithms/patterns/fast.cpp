...
int main(int argc, char *argv[]) {
	...
	// p is the current origin
	for(Point p : points)
	{
		// Sort by slope to origin
		...
		// Find slope-streaks within sorted array
		double currentSlope = 99999999;
		int streak = 0;
		for(Point ps : sortedPoints)
		{
			if(p.slopeTo(ps) == currentSlope)
			{
				streak++;
				// If line is found, render it
				if(streak >= 3)
				{
					render_line(screen, p, ps);
				}
			}
			else
			{
				streak = 1;
				currentSlope = p.slopeTo(ps);
			}
		}
	}
