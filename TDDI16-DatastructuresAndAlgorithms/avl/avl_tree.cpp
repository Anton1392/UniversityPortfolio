...
class AVL_Tree_Node {
	private:
		...
		static void balance_left(Node_Pointer &t);
		static void balance_right(Node_Pointer &t);
...
void AVL_Tree_Node<Comparable>::remove(const Comparable &x, Node_Pointer &t) 
{
	...
	cout << t->element << " processing" << endl;

	// Traversera vänster eller höger i trädet
	if (x < t->element) 
	{
		cout << "Removing from left" << endl;
		remove(x, t->left);
		
		// Om trädet blir obalanserat åt höger
		if(node_height(t->right) - node_height(t->left) >= 2)
		{
			balance_right(t);
		}
	...
	{
		cout << "Removing from right" << endl;
		remove(x, t->right);
		
		// Om trädet blir obalanserat åt vänster
		if(node_height(t->left) - node_height(t->right) >= 2)
		{
			balance_left(t);
		}
	...
	else 
	{
		...
		// Om t har ett barn
		// Flytta upp barnet till t's plats, helt enkelt.
		else if (t->left == nullptr)
		{
			cout << "One child to right, swapping with it." << endl;
			t = t->right;
			delete tmp;
		}
		else if(t->right == nullptr)
		{
			cout << "One child to left, swapping with it." << endl;
			t = t->left;
			delete tmp;
		}
		else
		{
			cout << "Two children, swapping & removing from right" << endl;
			tmp = find_min(t->right);

			t->element = tmp->element;

			remove(tmp->element, t->right);
	...
	template <typename Comparable>
void AVL_Tree_Node<Comparable>::balance_left(Node_Pointer &t) 
{
	cout << "Left balancing node: " << t->element << endl;
	// Om det vänstra barnet har större än eller jämn höjd än det högra: rotera en gång.
	if(node_height(t->left->left) >= node_height(t->left->right))
	{
		single_rotate_with_left_child(t);
	}
	// Annars krävs två roteringar om högra barnet har större höjd.
	else
	{
		double_rotate_with_left_child(t);
	}
	...
	template <typename Comparable>
void AVL_Tree_Node<Comparable>::balance_right(Node_Pointer &t) 
{
	cout << "Right balancing node: " << t->element << endl;
	// Om det högra barnet har större höjd eller jämnt mot det vänstra: rotera en gång.
	if(node_height(t->right->right) >= node_height(t->right->left))
	{
		single_rotate_with_right_child(t);
	}
	// Annars krävs två roteringar om vänstra barnet har större höjd.
	else
	{
		double_rotate_with_right_child(t);
	}
