...
class ConstraintParser < Parser
  ...
  def get_connector(constraint)
    if(constraint.class == Connector || constraint.class == ConstantConnector)
      constraint
    elsif constraint.is_a?(ArithmeticConstraint)
      constraint.out
    end
  end
...
c.forget_value('user')
c.user_assign(80)
puts "CELSIUS: " + c.value.to_s
puts "FAHRENHEIT: " + f.value.to_s
# f.user_assign(81)
# puts "C SHOULD BE NINE: " + c.value.to_s
